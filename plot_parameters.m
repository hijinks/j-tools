...
upper_bounds = nan(length(dir_search),3);
lower_bounds = nan(length(dir_search),3);
...
hold on;
gscatter(1:1:length(ag),ag,fan_categories', colors, symbols, '', 'off');
...
hold on;
gscatter(1:1:length(bg),bg,fan_categories', colors, symbols, '', 'off');
...
subplot(2,3,3);
lb = abs(cg-upper_bounds(:,1));
ub = abs(cg-lower_bounds(:,1));
errorbar(1:1:length(cg),cg, lb,ub);
hold on
gscatter(1:1:length(cg),cg,fan_categories', colors, symbols, '', 'off');
...
subplot(2,3,4);
gscatter(1:1:length(C1),C1,fan_categories', colors, symbols);
...
subplot(2,3,5);
gscatter(1:1:length(C2),C2,fan_categories', colors, symbols, '', 'off');
...
subplot(2,3,6);
gscatter(1:1:length(CV),CV,fan_categories', colors, symbols, '', 'off');
